#include <iostream>
#include <cstdlib>
#include <string>
#include <cstring>
#include <unistd.h>
#include <sys/types.h>
#include <sys/socket.h>
#include <arpa/inet.h>
#include <netdb.h>

// Note: Check for SIGPIPE and other socket related signals

bool is_number(const std::string& str)
{
    for (std::size_t i = 0; str[i]; ++i)
    {
        if (std::isdigit(str[i]) == 0) return false;
    }
    return true;
}

int main(int ac, char** av)
{
    // Check av
    if (ac < 3 || !is_number(static_cast<std::string>(av[2])))
    {
        std::cerr << "[ERROR] Parameters are not valie!" << std::endl;
        return  -1;
    }

    int port = std::atoi(av[2]);
    // Address and host info
    sockaddr_in server_addr;
    hostent *server;

    // Create a socket & get the file descriptor
    int sock_fd = socket(AF_INET,  SOCK_STREAM, 0);
    // Check if the ssocket is created
    if (sock_fd < 0)
    {
        std::cerr << "[ERROR] Socket cannot be created!" << std::endl;
        return -2;
    }

    std::cout << "[INFO] Socket has been created." << std::endl;

    // Get host information by name
    // gethostbyname is not thread-safe, checkout getaddrinfo
    server = gethostbyname(av[1]);
    if (!server)
    {
        std::cerr << "[ERROR] No such host!" << std::endl;
        return -3;
    }
    std::cout << "[INFO] Hostname is found." << std::endl;

    // Fill address fields before try to connect
    std::memset((char*)&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_port =  htons(port);

    // Check if there is an address of the host
    if (server->h_addr_list[0])
        std::memcpy((char*)server->h_addr_list[0], (char *)&server_addr.sin_addr, server->h_length);
    else
    {
        std::cerr << "[ERROR] There is no a valid address for that hostname!" << std::endl;
        return -5;
    }

    if (connect(sock_fd, (sockaddr*)&server_addr, sizeof(server_addr)) < 0)
    {
        std::cerr << "Connection cannot be established!" << std::endl;
        return -6;
    }
    std::cout << "[INFO] Connection established." << std::endl;

    char buf[4096] = "test!";
    std::string temp;
    // While user input is not empty or "quit"
    // Send data to the server
    // Wait for a message/response
    // Print the response
    do {
        // Clear buffer, get user input
        std::memset(buf, 0, 4096);
        std::cout  << "> " <<  std::flush;
        std::getline(std::cin, temp, '\n');
        std::strcpy(buf, temp.c_str());

        //  Check the input
        if (!strlen(buf))
            continue;
        else if (!strcmp(buf, "quit"))
            break;

        // Send the data thar buffer contains
        int bytes_send = send(sock_fd, &buf, (size_t)strlen(buf), 0);
        // Check if message  sendign is successful
        if (bytes_send < 0)
        {
            std::cerr << "[ERROR] Message cannot be sent!" << std::endl;
            break;
        }

        // Wait for a message
        int bytes_recv = recv(sock_fd, &buf, 4096, 0);

        // Check how many bytes recieved
        // If something gone wrong
        if (bytes_recv < 0)
        {
            std::cerr << "[ERROR] Message cannot be recived!" << std::endl;
        }
        // If there is  no data,  it means server is disconnected
        else if (bytes_recv == 0)
        {
            std::cout << "[INFO] Server is disconnected." << std::endl;
        }
        else
        {
            std::cout << "server> " << std::string(buf, 0, bytes_recv) << std::endl;
        }
    } while (true);

    // Close socket
    close(sock_fd);
    std::cout << "[INFO] Socket is closed." << std::endl;

    return 0;
}
